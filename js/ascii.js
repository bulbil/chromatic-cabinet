...
var ascii = (function() {
	...
	// modified to return strings from each channel individually
	function asciiChannelFromCanvas(canvas, options) {
		// Original code by Jacob Seidelin (http://www.nihilogic.dk/labs/jsascii/)
		// Heavily modified by Andrei Gheorghe (http://github.com/idevelop)
		...
		for (var y = 0; y < canvasHeight; y += 2) { // every other row because letters are not square
			for (var x = 0; x < canvasWidth; x++) {
				...
				// for each channel generate a string representation based on the pure
				// values and the corrected values
				for ( var i = 0; i < asciiStrings.length; i++ ){
					if (i < 3){
						var brightness = (contrastedColor[channels[i%3]] * perceptionShift[i]) / 255;
					} else {
						var brightness = contrastedColor[channels[i%3]] / 255;
					}
					var char = getCharacter(brightness);
					asciiStrings[i] += ( char === 'undefined') ? "" : char;
				}
	...
	return {
		...
		channelFromCanvas: function(canvas, options) {
			...
			return asciiChannelFromCanvas(canvas, options);
		}
	};
})();
